...
const filePath = path.join(process.cwd(), 'server/webScraping/outputs/sports_data.json')
...
export async function ingestSportsArticles() {
  try {
    ...
    LoggerService.info(`✅ Upserted ${vectors.length} sports articles.`)
  } catch (error) {
    LoggerService.error(`🆘 Failed to upsert sports: ${error}`)
  }
}
