...
jest.mock('./controllers/userQueryController', () => ({
  parseUserQuery: jest.fn((req, res, next) => {
    res.locals.userQuery = req.body.userQuery
    next()
  }),
}))

jest.mock('./controllers/openaiController', () => ({
  queryOpenAIEmbedding: jest.fn((_req, res, next) => {
    res.locals.userQueryEmbedding = [0, 1, 2]
    next()
  }),
  queryOpenAIChat: jest.fn((_req, res, next) => {
...
jest.mock('./controllers/pineconeController', () => ({
  queryPineconeDatabase: jest.fn((_req, res, next) => {
...
describe('POST /api', () => {
  it('should return 200 and the expected response', async () => {
    const response = await request(app)
      ...
      .send(
        JSON.stringify({
          userQuery: 'A genie who grants wishes, but the third frees him to unleash chaos until a woman traps him.',
        })
      )
      .set('Content-Type', 'application/json')

    expect(response.status).toBe(200)
    expect(response.body).toHaveProperty('movieRecommendation')
    expect(response.body.movieRecommendation).toBe(
      'Wishmaster - A malevolent genie wreaks havoc after being freed, leading to a battle between his dark desires and those trying to stop him.'
    )
  })
})
